...
# YES.md Safety Gate: Confirm Clear Queue
    ...
    with col_r:
        ...
        with st.form("score_form_v4", border=False):
            ...
            for i, row in enumerate(current_config['basic']):
                ...
                st.markdown(f"""
                <div class="score-item" style="margin-bottom: 0px; border-bottom: none; border-radius: 10px 10px 0 0;">
                    <div>
        ...
        if submitted:
            if not input_name:
                ...
                # YES.md Fix: 防呆，強制將最終分數限制在 0 ~ 100 區間，避免極端數值
        ...
        if st.session_state.calculated_score_data:
            ...
            with col_res1:
                ...
                <div class="result-panel">
                    ...
                    <div class="result-bonus-text">建議核發獎金 <strong style="color:var(--text-primary); font-size:15px;">{grade_m}</strong> 個月</div>
                </div>
                """, unsafe_allow_html=True)
...
# ==========================================
    ...
    with tab2:
        ...
        st.markdown('<div style="font-size:12px; color:var(--accent-teal); font-weight:700; margin: 12px 0 8px;">A 區細項 (KPI 基礎)</div>', unsafe_allow_html=True)
        # YES.md Fix: Ripple Check - 加入動態 key 避免 DuplicateWidgetID
        ...
        # YES.md Fix: 補回遺失的 B 區修改編輯器
        st.markdown('<div style="font-size:12px; color:var(--accent-blue); font-weight:700; margin: 12px 0 8px;">B 區細項 (OKR 挑戰)</div>', unsafe_allow_html=True)
