...
<style>
    ...
    .block-container { padding-top: 1.5rem !important; padding-bottom: 3rem !important; max-width: 96%; }
    ...
    .page-header-text h2 { margin: 0 0 5px 0; color: var(--text-primary); font-size: 22px; font-weight: 800; letter-spacing: 0.2px; }
    ...
    .section-label { display: flex; align-items: center; gap: 10px; font-size: 12px; font-weight: 700; color: var(--text-secondary); letter-spacing: 1.5px; text-transform: uppercase; margin: 22px 0 12px 0; padding-bottom: 10px; border-bottom: 1px solid var(--border); }
    .section-label span { display: inline-flex; align-items: center; justify-content: center; width: 22px; height: 22px; background: var(--accent-blue); border-radius: 6px; font-size: 11px; font-weight: 800; color: white; flex-shrink: 0; }
    ...
    .score-weight { font-size: 12px; color: var(--accent-blue); background: rgba(59,111,232,0.1); padding: 2px 6px; border-radius: 4px; font-family: 'DM Mono', monospace; }
    .score-help { font-size: 11px; color: var(--text-muted); margin-top: 4px; line-height: 1.4; }
    .score-help.warning { color: #D4820A; background: rgba(212,130,10,0.08); padding: 2px 6px; border-radius: 4px; display: inline-block;}

    /* ===== 結果看板 ===== */
    .result-panel { background: var(--bg-surface); border: 1px solid var(--border); border-radius: 14px; padding: 28px 24px; text-align: center; position: relative; overflow: hidden; box-shadow: 0 2px 12px rgba(59,111,232,0.06); height: 100%;}
...
if menu == "📝 新增評核":
    ...
    with col_l:
        ...
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown('<div style="font-size:12px; font-weight:700; color:var(--accent-teal); margin-bottom:10px; letter-spacing:1px;">▸ A. 基礎目標 (KPI)</div>', unsafe_allow_html=True)
        ...
        st.markdown('<div style="font-size:12px; font-weight:700; color:var(--accent-blue); margin:14px 0 10px; letter-spacing:1px;">▸ B. 挑戰目標 (OKR)</div>', unsafe_allow_html=True)
        ...
        with st.form("score_form_v35"):
            ...
            for i, row in enumerate(current_config['basic']):
                ...
                with c_lbl:
                    ...
                    """, unsafe_allow_html=True)
                with c_val:
                    st.markdown('<div style="margin-top: 5px;">', unsafe_allow_html=True) # 稍微往下推讓它垂直置中
...
# ==========================================
    ...
    with tab2:
        ...
        ed_a = st.data_editor(pd.DataFrame(conf['basic']), num_rows="dynamic", use_container_width=True, key=f"ed_a_{edit_dept}")
        ...
        ed_b = st.data_editor(pd.DataFrame(conf['excellent']), num_rows="dynamic", use_container_width=True, key=f"ed_b_{edit_dept}")
...
<div class="system-footer">
    ...
    <p style="font-size:10px;">系統版本 v35.1 - 全新不飽和美學與邏輯修復版</p>
</div>
""", unsafe_allow_html=True)
